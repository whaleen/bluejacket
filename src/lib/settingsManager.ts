...
export async function resolveLocationSettings(activeLocationKey: string): Promise<{
  ...
  const { data: settings, error: settingsError } = await supabase
    .from('settings')
    .select('sso_username, sso_password, ui_handedness, last_sync_asis_at, last_sync_fg_at, last_sync_sta_at, last_sync_inbound_at, last_sync_backhaul_at, last_sync_inventory_at, last_sync_orders_at')
