# GE DMS Exploration Guide for Agents
...
**CRITICAL**: The production sync service (`npm run sync`) must continue working during exploration. Never modify production code or credentials during exploration sessions.
...
- [ ] GE SSO credentials stored in Supabase `location_configs` table for location `00000000-0000-0000-0000-000000000001`
...
If credentials are missing, see `docs/auth.md` for setup instructions.
...
1. **`exploreLink.ts`** - Navigate to a page by clicking dashboard link
   - Usage: `npx tsx src/scripts/exploreLink.ts "Link Text"`
   - Example: `npx tsx src/scripts/exploreLink.ts "ASIS"`
   - Keeps browser open for continued exploration
   - Takes screenshot and saves page content to `/tmp/ge-*`
...
3. **`explore.ts`** - Generic exploration with custom navigation
   - Template for custom exploration logic
   - Modify as needed for specific tasks
...
- `services/ge-sync/docs/GE_DMS_SYSTEM_OVERVIEW.md` - Business processes and glossary
- `services/ge-sync/docs/GE_DMS_PAGES.md` - Complete page catalog (30 pages documented)
- `services/ge-sync/docs/GE_ENDPOINT_FIELDS.md` - Known data exports and field mappings
...
# If not running and you need it, start in separate terminal:
cd services/ge-sync && npm run sync
```
...
2. Verify `location_configs` table has entry for location ID `00000000-0000-0000-0000-000000000001`
3. Ensure `ssoUsername` and `ssoPassword` fields are populated
4. See `docs/auth.md` for detailed setup
...
3. Save to `/tmp/` directory for easy cleanup
...
ls -lh /tmp/asis-load-export-*.csv
```
...
2. **Check existing docs** (`GE_DMS_PAGES.md`, `GE_DMS_SYSTEM_OVERVIEW.md`)
...
- **GE DMS Pages Catalog**: `services/ge-sync/docs/GE_DMS_PAGES.md`
- **Business Processes**: `services/ge-sync/docs/GE_DMS_SYSTEM_OVERVIEW.md`
- **Field Mappings**: `services/ge-sync/docs/GE_ENDPOINT_FIELDS.md`
- **Auth Setup**: `services/ge-sync/docs/auth.md`
